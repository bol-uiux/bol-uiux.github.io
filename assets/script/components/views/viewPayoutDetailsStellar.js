function checkForm(form) {
    ...
    valid_address = (document.getElementById('AccountIdentifier').value.length > 50 ? true : false);

    if (!valid_address) {
		$('#TwoFactorAuthModal').modal('hide');
        jcount++;
		jmsgText=jmsgText + (jcount ==1 ? "Stellar Address" : ", Stellar Address");
	...
	if (jcount > 0) { 
		var error_confg = {
			title			: "Stellar Address Verification",
			corrections		: ["The following form fields are either invalid or missing. Please review. <br> <strong>Stellar Address</strong> "]
		};
