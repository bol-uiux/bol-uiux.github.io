function checkForm(form) {
    ...
    // if (isValidBitCoinAddress(form.AccountIdentifier.value)==false) {
    // Using better validation here...

    var valid_address = WAValidator.validate(form.AccountIdentifier.value, 'bitcoin');

    if (!valid_address) {
        $('#TwoFactorAuthModal').modal('hide');
        jcount++;
        jmsgText = jmsgText + (jcount == 1 ? "Bitcoin Address" : ", Bitcoin Address");
    ...
    if (jcount > 0) {
        ...
        var error_confg = {
            ...
            corrections: ["The following form fields are either invalid or missing. Please review. <br> <strong>Bitcoin Address</strong> "]
        };
...
$(document).ready(function () {
    ...
    function validateBTCaddress(e) {

        var valid_address;
        setTimeout(function () {
            valid_address = WAValidator.validate(e.target.value, 'bitcoin');
    ...
    document.getElementById("AccountIdentifier").addEventListener("keyup", validateBTCaddress, false);
    document.getElementById("AccountIdentifier").addEventListener("paste", validateBTCaddress, false);
    document.getElementById("AccountIdentifier").addEventListener("change", validateBTCaddress, false);
    document.getElementById("AccountIdentifier").addEventListener("paste", validateBTCaddress, false);
    document.getElementById("AccountIdentifier").addEventListener("touchstart", validateBTCaddress, false);
    document.getElementById("AccountIdentifier").addEventListener("touchend", validateBTCaddress, false);
