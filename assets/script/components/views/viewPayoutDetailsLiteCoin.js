function checkForm(form) {
    ...
    valid_address = WAValidator.validate(document.getElementById('AccountIdentifier').value, 'litecoin');

    if (!valid_address) {
		$('#TwoFactorAuthModal').modal('hide');
        jcount++;
		jmsgText=jmsgText + (jcount ==1 ? "Litecoin Address" : ", Litecoin Address");
	...
	if (jcount > 0) { 
		var error_confg = {
			title			: "Litecoin Address Verification",
			corrections		: ["The following form fields are either invalid Or missing. Please review. <br> <strong>Litecoin Address</strong> "]
		};
