function checkForm(form) {
	var legacyAddress = false;
	var loader = document.getElementById("app_load");
	var app_view = document.getElementById("app");
	var bchAddress = document.getElementById('AccountIdentifier').value;
	var sBookID = $('input[name ="SBookID"]').val();
    var paymentMethod = $('input[name ="PaymentMethod"]').val();

	if (bchAddress.substring(0,1) == "1") legacyAddress = true;
	if (bchAddress.indexOf(":") == -1) bchAddress = 'bitcoincash:' + bchAddress;
    ...
    //valid_address = WAValidator.validate(document.getElementById('AccountIdentifier').value, 'bitcoincash');

    try {
        cashaddr.decode(bchAddress);
        valid_address = true;
    }
    catch (e) { 
		if (legacyAddress)
			...
			console.log(bchAddress + ' invalid', e);
			valid_address = false; 
		}
	}
    ...
    if (!valid_address) {
        ...
        jcount++;
		jmsgText=jmsgText + (jcount ==1 ? "Bitcoin Cash Address" : ", Bitcoin Cash Address");
	...
	if (jcount > 0) {
		...
		var error_confg = {
			title			: "Bitcoin Cash Address Verification",
			corrections		: ["The following form fields are either invalid or missing. Please review. <br> <strong>Bitcoin Cash Address</strong> "]
		};
...
$(document).ready(function() {
	...
	function validateNetellerAccount(e){
		...
		if (bchAddress.substring(0,1) == "1") legacyAddress = true;
		if (bchAddress.indexOf(":") == -1)
			bchAddress = 'bitcoincash:' + bchAddress;
		...
		setTimeout(function() {
			...
			//valid_address = WAValidator.validate(document.getElementById('AccountIdentifier').value, 'bch');		
